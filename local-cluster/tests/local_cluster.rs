...
fn test_incremental_snapshot_download_with_crossing_full_snapshot_interval_at_startup() {
    ...
    loop {
        ...
        let validator_current_slot = cluster
            ...
            .unwrap();
        */
        let validator_client = cluster.get_validator_client(&validator_identity.pubkey()).unwrap();
        let validator_current_slot = validator_client.get_slot_with_commitment(CommitmentConfig::finalized());
        error!("<<<###>>> {validator_current_slot:?}");
        debug_assert!(validator_current_slot.is_ok(), "!!!!!! get_slot_result={validator_current_slot:?}");
