...
where
    F: Fn(Shred),
{
    let timer = Duration::from_millis(200); //herehere_magicval
    let (mut shreds, mut repair_infos) = shred_receiver.recv_timeout(timer)?; //herehere_mark
    ...
    for index in inserted_indices {
        if repair_infos[index].is_some() {
            metrics.num_repair += 1; //herehere_mark
        }
    }
...
struct RepairMeta {
    _from_addr: SocketAddr, //herehere_mark could be useful to track
    nonce: Nonce,
}
